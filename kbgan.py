...
class Component():
    ...
    def train(self, heads: torch.Tensor, tails: torch.Tensor, train_data: tuple, valid_data: tuple,
        ...
        if self.model_type in ['TransE', 'TransD']:
            corrupter = BernCorrupter(train_data, self.model.n_entity, self.model.n_relation)
        elif self.model_type in ['DistMult', 'ComplEx']:
            self.n_sample = self.model.model_config.n_sample
            corrupter = BernCorrupterMulti(train_data, self.model.n_entity, self.model.n_relation, self.n_sample)
        else:
